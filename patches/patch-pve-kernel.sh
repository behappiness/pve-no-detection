...
static u64 rdtsc_timediffdividersecond = '$DIVIDER_SECOND';\nstatic bool rdtsc_secondcall = false;' "$SVM"

sed -i '/MODULE_LICENSE("GPL");/a \
static u32 rdtsc_print_once = 1;\nstatic u64 rdtsc_timediffdivider = '$DIVIDER';' "$SVM"
...
# sed -i '/static int (\*const svm_exit_handlers\[\])(struct kvm_vcpu \*vcpu)/i \
# static int svm_handle_rdtsc(struct kvm_vcpu \*vcpu) {\n	static u64 rdtsc_fake = 0;\n	static u64 rdtsc_prev = 0;\n	u64 rdtsc_real = rdtsc();\n	if(rdtsc_print_once) {\n		printk("\[handle_rdtsc\] Fake rdtsc svm function is working\\n");\n		rdtsc_print_once = 0;\n		rdtsc_fake = rdtsc_real;\n	}\n	if(rdtsc_prev != 0)	{\n		if(rdtsc_real > rdtsc_prev)	{\n			u64 diff = rdtsc_real - rdtsc_prev;\n			u64 fake_diff = diff \/ rdtsc_timediffdivider;\n			rdtsc_fake += fake_diff;\n		}\n	}\n	if(rdtsc_fake > rdtsc_real) {\n		rdtsc_fake = rdtsc_real;\n	}\n	rdtsc_prev = rdtsc_real;\n	vcpu->arch.regs\[VCPU_REGS_RAX\] = rdtsc_fake & -1u;\n	vcpu->arch.regs\[VCPU_REGS_RDX\] = (rdtsc_fake >> 32) & -1u;\n	return svm_skip_emulated_instruction(vcpu);\n}' "$SVM"
...
sed -i '/__unused_size_checks();/a \
	rdtsc_sysctl_table_header = register_sysctl("kernel", rdtsc_sysctl_table);\n	if(!rdtsc_sysctl_table_header) {\n		pr_err("Failed to register sysctl table\\n");\n	}' "$SVM"
...
sed -i '/MODULE_LICENSE("GPL");/a \
static u32 rdtsc_print_once = 1;\nstatic u64 rdtsc_timediffdivider = '$DIVIDER';' "$VMX"
